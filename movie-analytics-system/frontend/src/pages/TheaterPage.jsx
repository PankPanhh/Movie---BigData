...
export default function TheaterPage() {
  ...
  const loadData = useCallback(async (page, currentSearch) => {
    try {
      const res = await fetchTheaters(page, 10, currentSearch);
      setData({
        items: res.data,
  ...
  const handleSave = async (formData) => {
    try {
      if (editingItem) {
        await updateTheater(editingItem._id, formData);
      } else {
  ...
  return (
    <div className="container mx-auto max-w-7xl">
      <h1 className="text-4xl font-bold text-gray-800 mb-8">Quản lý Rạp hát</h1>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="relative w-1/3">
          ...
          <input
            type="text"
            placeholder="Tìm kiếm theo thành phố, đường, tiểu bang..."
      ...
      <div className="bg-white shadow-xl rounded-xl overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-100">
            <tr>
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Đường</th>
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Thành phố</th>
              <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tiểu bang</th>
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {data.items.map((item) => (
              <tr key={item._id} className="hover:bg-blue-50 transition-colors duration-150">
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.location.address.street1}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{item.location.address.city}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{item.location.address.state}</td>
      ...
      {/* Modal Form */}
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingItem ? "Sửa rạp hát" : "Thêm rạp hát"}>
        <TheaterForm
